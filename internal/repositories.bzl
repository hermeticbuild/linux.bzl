...
_PROBE_ENV = {
    ...
    "BINDGEN": "bindgen",
    "CC": "clang",
    "CC_VERSION_TEXT": "clang version 22.1.4None",
...
_REPOSITORY_GENERATOR_PROTOCOL = "compact-v1-full-config"
...
def _resolve_config(rctx, tool, source_root, arch, version, name, raw):
...
def _validate_generated_metadata(rctx, graph_dir, config_name, source_root):
    ...
    for variant in variants:
        ...
        if mode != "y":
            fail(
                "Linux graph for config %s selects unsupported module object %s with mode %r" %
