...
type scannedSourceFile struct {
	...
	binaryIncludes []sourceBinaryIncludeDirective
	digest         string
}
...
type sourceBinaryIncludeDirective struct {
	line              int
	path              string
	spelling          string
	literal           bool
...
func (s sourceIncludeSearch) roots(kind sourceIncludeKind) []string {
	...
	roots = append(roots, s.afterRoots...)
	return roots
}
...
func (s *configSourceScanner) actionIncludeSearch(source string, flags []string) (sourceIncludeSearch, error) {
	flagSearch, err := sourceIncludeSearchFromFlags(flags, source)
	if err != nil {
...
func sourceIncludeSearchFromFlags(flags []string, source string) (sourceIncludeSearch, error) {
	...
	for _, flag := range pathFlags {
		...
		class, root, err := resolveSourceFlagPath(flag.path, source)
		if err != nil {
			return sourceIncludeSearch{}, fmt.Errorf("%s path %q: %w", flag.option, flag.path, err)
		}
		...
		case "-idirafter":
			search.afterRoots = appendUniqueIncludeRoots(search.afterRoots, root)
		default:
...
const (
	sourceFlagPathExternal sourceFlagPathClass = iota
	sourceFlagPathTree
	sourceFlagPathObject
)

func sourcePathFlags(flags []string) ([]sourcePathFlag, error) {
	options := []string{"-idirafter", "-iquote", "-isystem", "-include", "-imacros", "-I"}
	unsupported := []string{
		"-F",
		"-iframework",
		"-iprefix",
	...
	for i := 0; i < len(flags); i++ {
		...
		for _, option := range options {
			if flag == option {
				...
				result = append(result, sourcePathFlag{option: option, path: path})
				matched = true
				break
			}
			if path, ok := strings.CutPrefix(flag, option); ok && path != "" {
		...
		for _, prefix := range unsupported {
			if strings.HasPrefix(flag, prefix) {
				return nil, fmt.Errorf("unsupported source include flag %q", flag)
			}
		}
	}
	return result, nil
...
func resolveSourceFlagPath(path, source string) (sourceFlagPathClass, string, error) {
	...
	class := sourceFlagPathTree
	switch {
	case path == "$(srctree)", path == "${srctree}":
		path = ""
	case strings.HasPrefix(path, "$(srctree)/"):
		path = strings.TrimPrefix(path, "$(srctree)/")
	case strings.HasPrefix(path, "${srctree}/"):
		path = strings.TrimPrefix(path, "${srctree}/")
	case path == "$(src)", path == "${src}":
		path = sourceDir
	case strings.HasPrefix(path, "$(src)/"):
		path = filepath.ToSlash(filepath.Join(sourceDir, strings.TrimPrefix(path, "$(src)/")))
	case strings.HasPrefix(path, "${src}/"):
		path = filepath.ToSlash(filepath.Join(sourceDir, strings.TrimPrefix(path, "${src}/")))
	case path == "$(obj)", path == "${obj}":
		class = sourceFlagPathObject
		path = sourceDir
	case strings.HasPrefix(path, "$(obj)/"):
		class = sourceFlagPathObject
		path = filepath.ToSlash(filepath.Join(sourceDir, strings.TrimPrefix(path, "$(obj)/")))
	case strings.HasPrefix(path, "${obj}/"):
		class = sourceFlagPathObject
		path = filepath.ToSlash(filepath.Join(sourceDir, strings.TrimPrefix(path, "${obj}/")))
	case strings.Contains(path, "$(") || strings.Contains(path, "${"):
		return sourceFlagPathExternal, "", fmt.Errorf("contains an unresolved make variable")
	case filepath.IsAbs(filepath.FromSlash(path)):
		return sourceFlagPathExternal, "", nil
	}
	...
	if path == ".." || strings.HasPrefix(path, "../") || filepath.IsAbs(filepath.FromSlash(path)) {
		return sourceFlagPathExternal, "", fmt.Errorf("escapes the source tree")
	}
...
func (s *configSourceScanner) closureForSourceConfigInputsSearchProfile(
	...
	for len(stack) > 0 {
		...
		for _, inc := range scanned.binaryIncludes {
			if !inc.potentiallyActive {
				continue
			}
			if !inc.literal {
			...
			resolvedInputs := s.resolveInclude(treePath, inc.path, sourceIncludeQuoted, search)
			if len(resolvedInputs) == 0 {
				inputPath := filepath.ToSlash(strings.TrimSpace(inc.path))
				if provided[inputPath] {
					continue
				}
				err := fmt.Errorf(
					"%s:%d: unresolved potentially-active .incbin input %s",
					treePath,
					inc.line,
					inc.spelling,
				)
			...
			for _, resolved := range resolvedInputs {
				input, err := s.inputForTreePath(resolved)
				if err != nil {
					err = fmt.Errorf("%s:%d: %w", treePath, inc.line, err)
					s.closure[key] = sourceClosure{}
...
func (s *configSourceScanner) scanFile(
	...
	for _, line := range lines {
		...
		if !ok {
			...
			if assembly {
				path, literal, directive := assemblerIncbinOperand(line.text)
				if directive {
					scanned.binaryIncludes = append(
						scanned.binaryIncludes,
						sourceBinaryIncludeDirective{
							line:              line.line,
							path:              path,
							spelling:          strings.TrimSpace(line.text),
							literal:           literal,
...
func assemblerIncbinOperand(line string) (path string, literal, directive bool) {
	line = strings.TrimSpace(line)
	if !strings.HasPrefix(line, ".incbin") {
		return "", false, false
	}
	rest := strings.TrimPrefix(line, ".incbin")
	if rest != "" && !strings.ContainsAny(rest[:1], " \t\v\f\r\"") {
		return "", false, false
	}
	rest = strings.TrimSpace(rest)
	if rest == "" || rest[0] != '"' {
	...
	return filepath.ToSlash(value), true, true
}
