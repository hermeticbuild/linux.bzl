...
func TestIncludeDirsFromFlags(t *testing.T) {
	got, err := includeDirsFromFlags([]string{
		"-Wall",
	...
	if err != nil {
		t.Fatalf("includeDirsFromFlags() failed: %v", err)
	}
	want := []string{
		"drivers/foo",
		"include/generated",
...
func TestSourceIncludeFlagsFailClosed(t *testing.T) {
	...
	for _, flags := range [][]string{
		...
		if _, err := scanner.actionIncludeSearch("drivers/foo.c", flags); err == nil {
			t.Errorf("actionIncludeSearch(%v) succeeded, want an unsupported flag error", flags)
		}
	}

	for _, flags := range [][]string{
		{"-include", "/external/private.h"},
		{"-imacros", "$(obj)/unmodeled.h"},
	} {
		if _, err := forcedSourceInputs(flags, "drivers/foo.c", "drivers/foo.o"); err == nil {
			t.Errorf("forcedSourceInputs(%v) succeeded, want an unresolved input error", flags)
		}
	}
	if _, err := forcedSourceInputs(
		[]string{"-include", "$(obj)/utsversion-tmp.h"},
		"init/version.c",
		"init/version.o",
	); err != nil {
		t.Fatalf("forcedSourceInputs(utsversion-tmp.h) failed: %v", err)
	}
}
...
func TestConfigSourceScannerContentGraphTracksActiveAssemblerIncbin(t *testing.T) {
	root := t.TempDir()
	mustWriteSource(t, root, "arch/x86/entry.S", `
#ifdef CONFIG_EMBED
.incbin "payload/data.bin", 1, 2
#endif
#if 0
.incbin MISSING_PAYLOAD
#endif
`)
	mustWriteSource(t, root, "arch/x86/payload/data.bin", "first payload\n")

	scan := func(config *ResolvedConfig) sourceClosure {
		t.Helper()
		scanner := newConfigSourceScanner(CompactMetadataOptions{SourceRoot: root})
		closure, err := scanner.closureForSourceConfig("arch/x86/entry.S", nil, config)
		if err != nil {
	...
	off := &ResolvedConfig{
		Effective: map[string]string{"CONFIG_EMBED": "n"},
		Written:   map[string]bool{"CONFIG_EMBED": true},
	}
	offClosure := scan(off)
	if got, want := sourceInputPaths(offClosure.sourceInputs), []string{"arch/x86/entry.S"}; !reflect.DeepEqual(got, want) {
		t.Fatalf("disabled .incbin inputs = %v, want %v", got, want)
	}

	on := &ResolvedConfig{
		Effective: map[string]string{"CONFIG_EMBED": "y"},
		Written:   map[string]bool{"CONFIG_EMBED": true},
	}
	onClosure := scan(on)
	wantPaths := []string{"arch/x86/entry.S", "arch/x86/payload/data.bin"}
	if got := sourceInputPaths(onClosure.sourceInputs); !reflect.DeepEqual(got, wantPaths) {
		t.Fatalf("enabled .incbin inputs = %v, want %v", got, wantPaths)
	}
	firstDigest := onClosure.sourceInputs[1].Digest

	mustWriteSource(t, root, "arch/x86/payload/data.bin", "second payload\n")
	changedClosure := scan(on)
	if changedClosure.sourceInputs[1].Digest == firstDigest {
		t.Fatal("changed .incbin input retained its previous digest")
	}
}

func TestConfigSourceScannerContentGraphRejectsUnresolvedAssemblerIncbin(t *testing.T) {
	tests := []struct {
		name string
		line string
		want string
	}{
		{name: "computed", line: ".incbin CONFIG_PAYLOAD", want: ".incbin operand"},
		{name: "missing", line: `.incbin "missing.bin"`, want: ".incbin input"},
	}
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			root := t.TempDir()
			mustWriteSource(t, root, "drivers/payload.S", tc.line+"\n")
			scanner := newConfigSourceScanner(CompactMetadataOptions{SourceRoot: root})
			_, err := scanner.closureForSource("drivers/payload.S", nil)
			if err == nil {
				t.Fatal("closureForSource() succeeded, want an unresolved .incbin error")
			}
			for _, want := range []string{"drivers/payload.S:1", tc.want} {
				if !strings.Contains(err.Error(), want) {
					t.Errorf("closureForSource() error = %q, want substring %q", err, want)
				}
			}
...
func TestConfigSourceScannerContentGraphAllowsActionProvidedAssemblerIncbin(t *testing.T) {
	root := t.TempDir()
	mustWriteSource(
		t,
		root,
		"arch/x86/entry.S",
		".incbin \"arch/x86/generated.bin\"\n",
	)
	scanner := newConfigSourceScanner(CompactMetadataOptions{SourceRoot: root})
	closure, err := scanner.closureForSourceConfigInputsProfile(
		...
		nil,
		true,
		[]string{"arch/x86/generated.bin"},
		sourceScanKernel,
	)
	if err != nil {
		t.Fatalf("closureForSourceConfigInputsProfile() failed: %v", err)
	}
	if got, want := sourceInputPaths(closure.sourceInputs), []string{"arch/x86/entry.S"}; !reflect.DeepEqual(got, want) {
		t.Fatalf("action-provided .incbin source inputs = %v, want %v", got, want)
	}
}
...
`)
	...
	search, err := scanner.actionIncludeSearch("drivers/main.c", nil)
	if err != nil {
		t.Fatalf("actionIncludeSearch() failed: %v", err)
	}
...
`)
	...
	mustWriteSource(t, root, "system/late-ordered.h", "#define SYSTEM_ORDERED 1\n")
	mustWriteSource(t, root, "after/after-only.h", "#define AFTER 1\n")
	mustWriteSource(t, root, "after/late-ordered.h", "#define SHADOWED_AFTER 1\n")
	...
	flags := []string{
		...
		"-idirafter", "$(srctree)/after",
	}
	search, err := scanner.actionIncludeSearch("drivers/foo/main.c", flags)
	if err != nil {
		t.Fatalf("actionIncludeSearch() failed: %v", err)
	}
	closure, err := scanner.closureForSourceConfigInputsSearchProfile(
...
func TestForcedSourceInputs(t *testing.T) {
	flags := []string{
		...
		"-include", "drivers/utsversion-tmp.h",
	}

	got, err := forcedSourceInputs(flags, "drivers/foo.c", "drivers/foo.o")
	if err != nil {
		t.Fatalf("forcedSourceInputs(C) failed: %v", err)
	}
	want := []string{
		"drivers/private.h",
		"include/linux/compiler-version.h",
	...
	got, err = forcedSourceInputs(flags, "arch/x86/entry.S", "arch/x86/entry.o")
	if err != nil {
		t.Fatalf("forcedSourceInputs(assembly) failed: %v", err)
	}
	want = []string{
		"arch/x86/private.h",
		"drivers/utsversion-tmp.h",
		"include/linux/compiler-version.h",
		"include/linux/forced.h",
		"include/linux/joined.h",
		"include/linux/kconfig.h",
...
func sourceInputPaths(inputs []CompactSourceInput) []string {
